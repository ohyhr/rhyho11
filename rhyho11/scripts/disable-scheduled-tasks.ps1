...
function Toggle-Task($task, $enable) {
    ...
    $trustedinstaller_task_result = [int](C:\rhyho11\MinSudo.exe --NoLogo --TrustedInstaller --Privileged cmd /c "schtasks.exe /change $($toggle) /tn `"$($task)`" > nul 2>&1 && echo 0 || echo 1")
